    ...
    public function index()
    {
        ...
        foreach ($modules as $module) {
            if (file_exists(Module::getPath($module) . '/' . $module . '/Config/widgets.php')) {
                $widget_config = config($module->getLowerName() . ".widgets");
                if (!empty($widget_config) && is_array($widget_config)) {
        ...
        if (!empty($user_widgets)) {
            $decoded_widgets = json_decode($user_widgets->widgets);
            if (!empty($decoded_widgets) && is_object($decoded_widgets)) {
                $user_widgets = array_values((array)$decoded_widgets);
            } else {
                $user_widgets = [];
            }
        } else {
