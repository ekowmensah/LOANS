                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="float-right btn-group">
                                    @if($loan->client_type == 'group' && in_array($loan->status, ['approved', 'active', 'closed', 'written_off']))
                                        <a href="{{route('loan.member-allocations.index', $loan->id)}}" class="btn btn-info">
                                            <i class="fas fa-users"></i> Member Allocations
                                        </a>
                                    @endif
                                                            ...
                                                            <div class="modal-body">
                                                                ...
                                                                <div class="row">
                                                                    ...
                                                                    <div class="col-md-6">
                                                                        <div class="form-group">
                                                                            ...
                                                                            <div class="form-control-static">
                                                                                Every {{$loan->repayment_frequency}} {{ucfirst($loan->repayment_frequency_type)}}(s)
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                                ...
                                                                @if($loan->client_type == 'group')
                                                                    <div class="form-group">
                                                                                            ...
                                                                                            <tr>
                                                                                                ...
                                                                                                <td>
                                                                                                    <input type="number" 
                                                                                                           ...
                                                                                                           step="0.01" 
                                                                                                           min="0"
                                                                                                           data-member-id="{{ $member->client_id }}"
                                                                                                           placeholder="0.00">
                                                                                                    <input type="hidden" 
                                                                                                           name="member_allocations[{{ $member->client_id }}][client_id]" 
                                                                        ...
                                                                        <div class="row mb-3">
                                                                            <div class="col-md-6">
                                                                                <button type="button" id="distribute-evenly" class="btn btn-secondary">
                                                                            ...
                                                                            <div class="col-md-6">
                                                                                <div class="alert alert-info mb-0">
                                                                                    ...
                                                                                    Total allocation must equal the approved loan amount.
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                            ...
                            <div class="col-sm-4 col-md-4">
                                <table class="table table-striped table-bordered ">
                                    ...
                                    @if(!empty($loan->loan_provisioning))
                                        <tr>
                                            <th class="table-bold-loan">{{ trans_choice('loan::general.loan_provisioning',1) }}</th>
                                            <td>

                                                {{$loan->loan_provisioning->name}}
                                                ({{number_format($loan->principal*$loan->loan_provisioning->percentage/100,2)}}
                                                )
                                    ...
                                    <div class="m-4">
                                        <div class="btn-group">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-left">
                                                @can('loan.loans.schedules.edit')
                                                <a href="{{url('loan/'.$loan->id.'/schedule/edit')}}"
                                                   class="dropdown-item">
                                                    <i class="fas fa-edit"></i>
                                                    {{trans_choice('core::general.edit',1)}} {{trans_choice('loan::general.schedule',1)}}
                                                </a>
                                                @endcan
    ...
    $(document).ready(function() {
        ...
        function updateAllocationTotals() {
            ...
            $('.allocation-amount').each(function() {
                ...
                totalExpectedPayment += memberExpectedPayment;
        ...
        $(document).on('click', '#distribute-evenly', function() {
            let approvedAmount = parseFloat($('#approved_amount').val()) || 0;
            let memberCount = $('.allocation-amount').length;
            
            if (memberCount > 0 && approvedAmount > 0) {
                let equalAmount = (approvedAmount / memberCount).toFixed(2);
                $('.allocation-amount').val(equalAmount);
                updateAllocationTotals();
            } else if (approvedAmount <= 0) {
                alert('Please enter an approved amount first.');
            }
        });
        
        // Form validation for group loans
        $('form[action*="approve_loan"]').on('submit', function(e) {
            if ($('#member-allocations').length > 0) {
                let totalAmount = parseFloat($('#total-allocation').text()) || 0;
                let approvedAmount = parseFloat($('#approved_amount').val()) || 0;
                
                if (Math.abs(totalAmount - approvedAmount) > 0.01) {
                    e.preventDefault();
                    alert('Total member allocation must equal the approved loan amount.');
                    return false;
                }
            }
        });
