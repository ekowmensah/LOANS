    ...
    public function collection_sheet(Request $request)
    {
        ...
        if (!empty($start_date)) {
            ...
            $individual_loans = DB::table("loan_repayment_schedules")
                ...
                ->when($age_from && !$age_to, function ($query) use ($age_from) {
                    $query->whereRaw('TIMESTAMPDIFF(YEAR, clients.dob, CURDATE())=?', [$age_from]);
                })
                ->when($age_from && $age_to, function ($query) use ($age_from, $age_to) {
            ...
            if ($request->download) {
                if ($request->type == 'pdf') {
                    $pdf = PDF::loadView(theme_view_file('loan::report.repayment_pdf'), compact('start_date',
                        'end_date', 'branch_id', 'data', 'branches','gender','age_from','age_to'));
            ...
            if ($request->download) {
                if ($request->type == 'pdf') {
                    $pdf = PDF::loadView(theme_view_file('loan::report.arrears_pdf'), compact('start_date',
                        'end_date', 'branch_id', 'data', 'branches','gender','age_from','age_to'))->setPaper('A4', 'landscape');
            ...
            if ($request->download) {
                if ($request->type == 'pdf') {
                    $pdf = PDF::loadView(theme_view_file('loan::report.disbursement_pdf'), compact('start_date',
                        'end_date', 'branch_id', 'data', 'branches', 'loan_officer_id', 'loan_product_id', 'loan_products', 'users', 'status','gender','age_from','age_to'))->setPaper('A4', 'landscape');
                    return $pdf->download(trans_choice('loan::general.disbursement', 1) . '(' . $start_date . '  to ' . $end_date . ').pdf');
                }
                $view = theme_view('loan::report.arrears_pdf',
                    compact('start_date',
                        'end_date', 'branch_id', 'data', 'branches', 'loan_officer_id', 'loan_product_id', 'loan_products', 'users', 'status','gender','age_from','age_to'));
                if ($request->type == 'excel_2007') {
